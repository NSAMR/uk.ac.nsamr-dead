...
if(!$disable_scalia_style) {
	if(!$my_query->have_posts()) {
		return;
	}
?>
<div class="sc-post-grid <?php if($effects_enabled): ?>lazy-loading<?php endif;?>" >
	<?php while($my_query->have_posts()) : $my_query->the_post(); ?>
		<div class="sc-post-griid-item clearix <?php if($effects_enabled): ?>lazy-loading-item<?php endif;?>" <?php if($effects_enabled): ?>data-ll-effect="fading"<?php endif;?>>
			<div class="sc-post-grid-item-image">
				<?php echo '<a href="'.get_the_permalink().'">'; scalia_post_thumbnail('scalia-post-thumb', true, 'img-circle'); echo '</a>'; ?></div>
			<div class="sc-post-grid-item-info">
				<div class="sc-post-grid-item-title"><?php the_title('<a href="'.get_the_permalink().'">', '</a>'); ?></div>
				<div class="sc-post-grid-item-date"><?php echo get_the_date(); ?></div>
			</div>
		</div>
			...
			<ul class="wpb_thumbnails wpb_thumbnails-fluid vc_clearfix"
				...
				<?php foreach ( $posts as $post ): ?>
					...
					<li class="isotope-item <?php echo apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $li_span_class, 'vc_teaser_grid_li', $atts ) . $post->categories_css ?>"<?php echo $block_style ?>>
						<div class="isotope-inner"><div class="clearfix isotope-inner-clearfix">
