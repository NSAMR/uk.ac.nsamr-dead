...
if($vc_manager->mode() == 'admin_frontend_editor' || $vc_manager->mode() == 'admin_page' || $vc_manager->mode() == 'page_editable') {
	...
	$element = 'sc-tabs sc-tabs-style-'.$style;
	if ( 'vc_tour' == $this->shortcode ) $element = 'sc-tour sc-tour-style-'.$style;
	if ( isset( $matches[1] ) ) {
		$tab_titles = $matches[1];
	}
	$tabs_nav = '';
	$tabs_nav .= '<ul class="sc_tabs_nav ui-tabs-nav resp-tabs-list'.('vc_tour' == $this->shortcode ? ' clearfix' : '').'">';
	foreach ( $tab_titles as $tab ) {
		$tab_atts = shortcode_parse_atts($tab[0]);
		if(isset($tab_atts['title'])) {
	...
	$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, trim( $element . ' sc_content_element ' . $el_class ), $this->settings['base'], $atts );

	$output .= "\n\t" . '<div class="preloader"></div><div id="'.(!empty($id) ? sanitize_title($id) : 'sc-tabs-'.time() . rand( 0, 100 )).'" class="' . $css_class . '" data-interval="' . $interval . '">';
	$output .= "\n\t\t" . '<div class="sc_wrapper sc_tour_tabs_wrapper ui-tabs clearfix">';
	$output .= wpb_widget_title( array( 'title' => $title, 'extraclass' => $element . '_heading' ) );
	$output .= "\n\t\t\t" . $tabs_nav;
	$output .= "\n\t\t\t" . '<div class="resp-tabs-container">' . wpb_js_remove_wpautop( $content ) . '</div>';
	$output .= "\n\t\t" . '</div> ' . $this->endBlockComment( '.sc_wrapper' );
	$output .= "\n\t" . '</div> ' . $this->endBlockComment( $element );
}
